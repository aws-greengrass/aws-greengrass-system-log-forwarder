...
GglError aws_sigv4_cloudwatch_post_header(
    ...
    static uint8_t headers_buffer[2048];
    GglByteVec headers_to_sign = { .buf = { .data = headers_buffer, .len = 0 },
                                   .capacity = sizeof(headers_buffer) };
    ...
    if (err == GGL_ERR_OK) {
        err = aws_sigv4_add_header_for_signing(
            &headers_to_sign, GGL_STR("date"), date_buf
        );
    }
    ...
    if (err == GGL_ERR_OK) {
        err = aws_sigv4_add_header_for_signing(
            &headers_to_sign,
            GGL_STR("host"),
            GGL_STR("logs.us-west-2.amazonaws.com")
        );
    }

    if (err == GGL_ERR_OK) {
        err = aws_sigv4_add_header_for_signing(
            &headers_to_sign, GGL_STR("user-agent"), GGL_STR("my-platform-name")
        );
    }

    if (err == GGL_ERR_OK) {
        err = aws_sigv4_add_header_for_signing(
            &headers_to_sign, GGL_STR("x-amz-date"), date_buf
        );
    }
